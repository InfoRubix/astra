...
/**
 * BranchAdminLayout - Layout wrapper for branch-level admin dashboards.
 *
 * Renders a responsive sidebar with collapsible sub-menus scoped to
 * the branch admin role (employee management, requests, reports),
 * an app bar with a profile menu, and child routes via <Outlet />.
 *
 * Redirects to /login if the current user is not a branch_admin.
...
function BranchAdminLayout() {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(() => {
    const saved = localStorage.getItem('branchAdminSidebarOpen');
    return saved !== null ? JSON.parse(saved) : true;
  });

  // Submenu state management
  const [openSubmenus, setOpenSubmenus] = useState(() => {
    const saved = localStorage.getItem('branchAdminOpenSubmenus');
  ...
  // Organized menu structure with submenus for Branch Admin
  const menuStructure = [
    { 
      text: 'Dashboard', 
      icon: <Dashboard />, 
    ...
    },
    {
      ...
      items: [
        { text: 'Branch Employees', icon: <People />, path: '/branch-admin/employees' }
      ]
    },
    {
      text: 'Requests',
      ...
      key: 'requests',
      items: [
        { text: 'Leave Management', icon: <EventAvailable />, path: '/branch-admin/leaves' },
        { text: 'Claims Management', icon: <Receipt />, path: '/branch-admin/claims' },
        { text: 'Forgotten Check-outs', icon: <AccessTime />, path: '/branch-admin/forgotten-checkouts' }
      ]
    },
    {
      ...
      key: 'reportsComms',
      items: [
        { text: 'Branch Reports', icon: <Assessment />, path: '/branch-admin/reports' },
        { text: 'Announcements', icon: <Announcement />, path: '/branch-admin/announcements' }
      ]
    }
  ];

  // Persist submenu state based on current active page
  useEffect(() => {
    const currentPath = location.pathname;
    let shouldOpenSubmenu = null;
    
    menuStructure.forEach(item => {
      if (item.type === 'group' && item.items) {
        const isActive = item.items.some(subItem => currentPath === subItem.path);
        if (isActive && !openSubmenus[item.key]) {
          shouldOpenSubmenu = item.key;
        }
      }
    });

    if (shouldOpenSubmenu) {
      const newOpenSubmenus = { ...openSubmenus, [shouldOpenSubmenu]: true };
      setOpenSubmenus(newOpenSubmenus);
      localStorage.setItem('branchAdminOpenSubmenus', JSON.stringify(newOpenSubmenus));
    }
  }, [location.pathname, openSubmenus, menuStructure]);
  ...
  const handleSidebarToggle = () => {
    ...
    localStorage.setItem('branchAdminSidebarOpen', JSON.stringify(newState));
  };
  ...
  const handleSubmenuToggle = (key) => {
    ...
    localStorage.setItem('branchAdminOpenSubmenus', JSON.stringify(newState));
  };
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      >
        <Toolbar sx={{ minHeight: { xs: 56, sm: 64 } }}>
          ...
          >
            {user.company} - {getBranchName()} Branch Administration
          </Typography>
      ...
      >
        <MenuItem 
          onClick={() => { navigate('/branch-admin/profile'); handleProfileMenuClose(); }}
          sx={{
            borderRadius: 1,
            mx: 1,
            mb: 0.5,
        ...
        >
          <ListItemIcon sx={{ minWidth: 36 }}>
            <AccountCircle fontSize="small" sx={{ color: '#666666' }} />
          </ListItemIcon>
      ...
      >
        ...
        >
          ...
          <Box sx={{ 
            ...
            {/* Navigation Items - Mobile always shows full content */}
            <Box sx={{ flex: 1, py: 1 }}>
              <List sx={{ px: 1 }}>
                    ...
                    {item.type === 'single' ? (
                      ...
                      <ListItem disablePadding sx={{ mb: 0.5 }}>
                        <ListItemButton
                          onClick={() => {
                            navigate(item.path);
                            handleDrawerToggle(); // Close mobile drawer on navigation
                          }}
                          sx={{
                            minHeight: 48,
                            borderRadius: 2,
                            ...
                            position: 'relative',
                            overflow: 'hidden',
                            color: location.pathname === item.path ? '#000000' : 'rgba(255, 255, 255, 0.8)',
                            transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                            '&:hover': {
                              backgroundColor: 'rgba(255, 255, 255, 0.08)',
                              color: 'white',
                      ...
                      // Group menu item with submenu - Mobile version
                      <Fragment>
                        <ListItem disablePadding sx={{ mb: 0.5 }}>
                          <ListItemButton
                            ...
                            sx={{
                              ...
                              position: 'relative',
                              overflow: 'hidden',
                              color: isSubmenuActive(item.items) ? '#000000' : 'rgba(255, 255, 255, 0.7)',
                              transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                              '&:hover': {
                          ...
                          >
                            ...
                            <ListItemText
                              primary={item.text}
                              primaryTypographyProps={{
                                fontWeight: isSubmenuActive(item.items) ? 600 : 500,
                                fontSize: '0.85rem',
                              ...
                              <ListItem key={subItem.text} disablePadding sx={{ mb: 0.5 }}>
                                <ListItemButton
                                  ...
                                  sx={{
                                    ...
                                    px: 2,
                                    position: 'relative',
                                    color: location.pathname === subItem.path ? '#000000' : 'rgba(255, 255, 255, 0.7)',
                                    transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
                                    '&:hover': {
                                      backgroundColor: 'rgba(255, 255, 255, 0.08)',
                                      color: 'rgba(255, 255, 255, 0.9)',
