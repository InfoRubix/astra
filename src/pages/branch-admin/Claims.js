...
function BranchAdminClaims() {
  ...
  useEffect(() => {
    if (user && user?.role === 'branch_admin') {
      console.log('🔍 Branch Admin Claims useEffect - User object:', {
        uid: user.uid,
        email: user.email,
      ...
      console.log('🔍 Branch Admin Claims useEffect - No user yet or not branch admin');
    }
  }, [user]);
  ...
  const loadBranchClaims = async () => {
    ...
    try {
      ...
      // Load claims only for this company (following company admin pattern)
      const q = query(
      ...
      console.log('🔍 Total claims loaded for company:', allClaims.length);
      
      // Filter claims for this branch only
      const branchClaims = allClaims.filter(claim => {
        // Check multiple possible branch field combinations for compatibility
        return claim.branchName === branchName || 
               claim.branch === branchName ||
               claim.branchId === user.branchId ||
               (claim.branchName && claim.branchName.includes(branchName)) ||
               (claim.branch && claim.branch.includes(branchName));
      });
      
      console.log('🏢 Branch Claims - Total company claims:', allClaims.length);
      console.log('🏢 Branch Claims - Branch claims:', branchClaims.length);
      console.log('🏢 Branch Claims - Branch name:', branchName);
      
      // Sort by submitted date (most recent first)
      branchClaims.sort((a, b) => {
      ...
      setClaims(branchClaims);
      console.log('🔍 Branch claims set for branch admin view');
    } catch (error) {
      console.error('Error loading branch claims:', error);
    ...
    try {
      ...
      const updateData = {
        ...
        ...(actionType === 'approve' && { 
          ...
          adminComments: actionReason || 'Approved by branch admin'
        })
      };
      ...
      const notification = {
        ...
        message: actionType === 'approve' 
          ? `Your ${selectedClaim.category} claim (${selectedClaim.currency} ${selectedClaim.amount}) has been approved by branch admin`
          : `Your ${selectedClaim.category} claim has been rejected by branch admin. ${actionReason ? 'Reason: ' + actionReason : ''}`,
        priority: 'high',
        read: false,
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const success = await pdfService.createProfessionalPDF({
        title: 'Branch Claims Management Report',
        data: filteredClaims,
        columns,
        filters,
        orientation: 'landscape',
        filename: pdfService.generateFilename('branch_claims_report', filters),
        additionalInfo: {
      ...
      if (success) {
        setSuccess('Branch claims report exported successfully!');
        setTimeout(() => setSuccess(''), 3000);
      } else {
  ...
  const handlePrintClaim = async (claim) => {
    ...
    try {
      ...
      doc.text('Branch Claim Details Report', 20, 25);
      ...
      doc.text(`Generated by: ${user.firstName} ${user.lastName} (Branch Admin)`, 20, 58);
      doc.text(`Branch: ${getBranchName()}`, 20, 64);
      ...
      // Save the PDF
      const filename = `branch_claim_${claim.id}_${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(filename);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 2, sm: 3 } }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box>
              ...
              >
                Manage expense claims for {getBranchName()}
              </Typography>
            </Box>
          </Box>
          <Button 
        ...
        <Box sx={{ display: { xs: 'block', md: 'none' } }}>
          {paginatedClaims.length === 0 ? (
            <Box sx={{ 
              ...
              <Typography variant="body2" color="text.secondary">
                {getTabData()[tabValue].status === 'pending' ? 'New expense claims from branch employees will appear here for your review' : 
                 ...
                 'Branch employee expense claims will appear here once submitted'}
              </Typography>
            </Box>
          ) : (
            <Box sx={{ p: 2 }}>
      ...
      {/* Action Dialog - Same as company admin but with branch context */}
      <Dialog 
        open={actionDialog} 
        onClose={() => {
          setActionDialog(false);
      ...
      >
        <DialogTitle>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            {actionType === 'view' ? 'Branch Claim Details' : 
             actionType === 'approve' ? 'Approve Branch Claim' : 'Reject Branch Claim'}
          </Box>
        </DialogTitle>
        <DialogContent>
          {selectedClaim && (
            <Box>
              {actionType === 'approve' && (
                <Alert 
                  ...
                  sx={{ mb: 2 }}
                >
                  You are about to approve this expense claim as branch admin. The employee will be notified via email.
                </Alert>
              )}
              {actionType === 'reject' && (
                <Alert 
                  ...
                  sx={{ mb: 2 }}
                >
                  You are about to reject this expense claim as branch admin. The employee will be notified via email.
                </Alert>
              )}
              ...
              <Box sx={{ p: 2, bgcolor: 'grey.50', borderRadius: 1, mb: 2 }}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <Typography variant="body2">
                      <strong>Branch:</strong> {selectedClaim.branchName || getBranchName()}
                    </Typography>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <Typography variant="body2">
              ...
              {actionType === 'approve' && (
                <Box>
                  <TextField
                    margin="dense"
                    label="Branch Admin Comments (Optional)"
                    fullWidth
                    variant="outlined"
                    value={actionReason}
                    onChange={(e) => setActionReason(e.target.value)}
...
export default BranchAdminClaims;