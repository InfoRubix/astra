...
function BranchAdminForgottenCheckouts() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      console.log('👤 Current branch admin user:', user);
      console.log('🏢 Company:', user.originalCompanyName || user.company);
      console.log('🏬 Branch:', user.branchName || user.branch);
      loadRequests();
      loadStats();
    }
  }, [user, statusFilter]);
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      // Load requests for this specific company and branch only
      const allRequests = await forgottenCheckoutService.getForgottenCheckoutRequests(userCompany, statusFilter);
      
      // Filter requests to only show those from the same branch
      const branchRequests = allRequests.filter(request => {
        const requestBranch = request.branchName || request.branch;
        return requestBranch === userBranch;
      });
      
      console.log('Loaded branch requests:', branchRequests);
  ...
  const loadStats = async () => {
    try {
      ...
      // Load stats for this specific company and branch only
      const companyStats = await forgottenCheckoutService.getForgottenCheckoutStats(userCompany, 30);
      
      // Filter stats to only include requests from the same branch
      const allRequests = await forgottenCheckoutService.getForgottenCheckoutRequests(userCompany, 'all');
      const branchRequests = allRequests.filter(request => {
        const requestBranch = request.branchName || request.branch;
        return requestBranch === userBranch;
      });
      ...
      const branchStats = {
        ...
        pendingRequests: branchRequests.filter(r => r.status === 'pending').length,
        approvedRequests: branchRequests.filter(r => r.status === 'approved').length,
        rejectedRequests: branchRequests.filter(r => r.status === 'rejected').length,
        approvalRate: branchRequests.length > 0 ? 
          Math.round((branchRequests.filter(r => r.status === 'approved').length / branchRequests.length) * 100) : 0
      };
      
      console.log('Loaded branch stats:', branchStats);
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box>
              ...
              >
                Review and process forgotten check-out requests for {user.branchName || user.branch || 'your branch'}
              </Typography>
            </Box>
          </Box>
        </Box>
              ...
              <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', flexDirection: { xs: 'column', sm: 'row' } }}>
                  <Avatar sx={{ 
                    ...
                    <PendingActions sx={{ fontSize: { xs: 20, sm: 24 } }} />
                  </Avatar>
              ...
              <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                <Box sx={{ display: 'flex', alignItems: 'center', flexDirection: { xs: 'column', sm: 'row' } }}>
                  <Avatar sx={{ 
                    ...
                    <Approval sx={{ fontSize: { xs: 20, sm: 24 } }} />
                  </Avatar>
                  <Box sx={{ textAlign: { xs: 'center', sm: 'left' } }}>
                    <Typography 
                      variant="h5" 
      ...
      >
        <Box sx={{ p: 3 }}>
          <Typography variant="h6" sx={{ fontWeight: 600, color: 'primary.main', mb: 3 }}>
            Branch Forgotten Check-out Requests
          </Typography>
          <Tabs 
            value={tabValue} 
          ...
          {loading ? (
            ...
            <Box sx={{ 
              ...
              <Typography variant="body2" color="text.secondary">
                {statusFilter === 'pending' ? 'New forgotten checkout requests from your branch will appear here for review' : 
                 statusFilter === 'approved' ? 'Approved requests from your branch will be shown here' :
                 statusFilter === 'rejected' ? 'Rejected requests from your branch will be displayed here' :
                 'Employee forgotten checkout requests from your branch will appear here once submitted'}
              </Typography>
            </Box>
          ) : (
            <Box sx={{ p: 2 }}>
...
export default BranchAdminForgottenCheckouts;