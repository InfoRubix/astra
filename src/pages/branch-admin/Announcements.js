...
function BranchAdminAnnouncements() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      console.log('🔍 Branch Admin Announcements useEffect - User object:', {
        uid: user.uid,
        email: user.email,
  ...
  const loadAnnouncements = async () => {
    ...
    try {
      ...
      // Load announcements for this branch only (branch-specific announcements)
      const q = query(
      ...
      const announcementsList = querySnapshot.docs
        ...
        .filter(announcement => {
          // Show branch-specific announcements or general company announcements that don't have a specific branch
          const annBranch = announcement.branchName || announcement.branch;
          return !annBranch || annBranch === userBranch;
        });
      ...
      console.log('🔍 Total announcements loaded for branch:', sortedAnnouncements.length);
  ...
  const loadAvailableDepartments = async () => {
    try {
      ...
      // Load all users from the same company and branch to get unique departments
      ...
      console.log('🔍 Available departments for branch:', departmentsList);
    } catch (error) {
  ...
  const createNotificationsForAnnouncement = async (announcement, announcementId) => {
    try {
      ...
      usersSnapshot.docs.forEach(doc => {
        ...
        // Skip admin users and branch-admin users
        if (userData.role === 'admin' || userData.role === 'branch-admin') return;
        ...
        if (userCompanyName.toUpperCase() === userCompany.toUpperCase() && 
            userBranchName === userBranch) {
          if (announcement.visibility === 'branch') {
            shouldNotify = true;
        ...
        if (shouldNotify) {
          notifications.push({
            ...
            relatedData: {
              ...
              branchName: announcement.branchName,
      ...
      if (notifications.length > 0) {
        ...
        await batch.commit();
        console.log(`Created ${notifications.length} notifications for branch announcement`);
      }
    } catch (error) {
  ...
  const handleCreateAnnouncement = async () => {
    ...
    try {
      ...
      const newAnnouncement = {
        ...
        company: userCompany,
        branchName: userBranch, // Always set to branch admin's branch
        createdBy: user.uid,
      ...
      console.log('Branch announcement created:', announcementRef.id);

      // Create notifications for affected users
      await createNotificationsForAnnouncement(newAnnouncement, announcementRef.id);

      setSuccess('Branch announcement created successfully and notifications sent to users');
  ...
  const handleEdit = () => {
    if (selectedAnnouncement) {
      ...
      setForm({
        ...
        branchName: selectedAnnouncement.branchName || '',
  ...
  return (
    <Container maxWidth="lg" sx={{ py: { xs: 2, sm: 3 } }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box>
              ...
              >
                Manage announcements for {user.branchName || user.branch || 'your branch'}
              </Typography>
            </Box>
          </Box>
          <Fab 
        ...
        <Box sx={{ p: 4 }}>
          ...
          {currentAnnouncements.length === 0 ? (
            <Box sx={{ textAlign: 'center', py: 4 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                Create your first branch announcement to get started.
              </Typography>
            </Box>
          ) : (
            <>
      ...
      <Dialog open={createDialog} onClose={() => setCreateDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>Create New Branch Announcement</DialogTitle>
        <DialogContent>
      ...
      <Dialog open={editDialog} onClose={() => {
        ...
        <DialogTitle>Edit Branch Announcement</DialogTitle>
        <DialogContent>
          <Grid container spacing={3} sx={{ mt: 1 }}>
            <Grid item xs={12}>
              <TextField
...
export default BranchAdminAnnouncements;