...
function AdminClaims() {
  ...
  useEffect(() => {
    if (user) {
      console.log('🔍 Admin Claims useEffect - User object:', {
        uid: user.uid,
        email: user.email,
      ...
      console.log('🔍 Admin Claims useEffect - No user yet');
    }
  }, [user]);
  ...
  const loadClaims = async () => {
    setLoading(true);
    try {
      console.log('🔍 Admin loading ALL claims (no company filtering)');
      
      // Load ALL claims from Firestore - admin sees all companies
      const q = query(collection(db, 'claims'));
      
      const querySnapshot = await getDocs(q);
      const allClaims = querySnapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
      }));
      ...
      console.log('🔍 Claims by company:', companySummary);
      ...
      setClaims(allClaims);
      console.log('🔍 All claims set for admin view');
    } catch (error) {
    ...
    try {
      ...
      const updateData = {
        ...
        ...(actionType === 'approve' && { 
          ...
          adminComments: actionReason || 'Approved by admin'
        })
      };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const success = await pdfService.createProfessionalPDF({
        title: 'Claims Management Report',
        data: filteredClaims,
        columns,
        filters,
        orientation: 'landscape',
        filename: pdfService.generateFilename('claims_report', filters),
        additionalInfo: {
  ...
  const handlePrintClaim = async (claim) => {
    ...
    try {
      ...
      doc.text(`Generated by: ${user.firstName} ${user.lastName}`, 20, 58);
      ...
      const checkPageBreak = (requiredHeight) => {
        ...
        if (yPos + requiredHeight > pageHeight - 40) { // 40mm margin from bottom
          doc.addPage();
          yPos = 20;
          return true;
        }
      ...
      const getImageDimensions = (base64Data, maxWidth = 160, maxHeight = 100) => {
        // Create a temporary image to get dimensions
        return new Promise((resolve) => {
          const img = new Image();
          img.onload = function() {
            const aspectRatio = this.width / this.height;
            let width = Math.min(maxWidth, this.width);
          ...
          img.onerror = function() {
            ...
            resolve({ width: maxWidth * 0.5, height: maxHeight * 0.5 });
          };
          img.src = base64Data;
        });
      };
      
      // Check for receipts data (supporting both possible field names)
      const receipts = claim.receipts || claim.receiptData || [];
      
      // Debug: Log receipt data structure with claim info
      const claimDate = claim.submittedDate?.toDate ? claim.submittedDate.toDate() : new Date(claim.submittedDate);
      const isOldClaim = claimDate < new Date('2025-01-28'); // Before the fix date
      
      console.log('🔍 Receipt data for claim:', claim.id, {
        submittedDate: claimDate.toISOString(),
        isOldClaim,
        totalReceipts: receipts.length,
        receiptStructure: receipts.length > 0 ? receipts.map((r, i) => ({
          index: i,
          name: r.name,
          hasData: !!r.data,
          dataType: typeof r.data,
          dataLength: r.data ? r.data.length : 0,
          dataPrefix: r.data ? r.data.substring(0, 50) + '...' : 'No data',
          isLikelyOldFormat: !r.data || typeof r.data !== 'string'
        })) : 'No receipts'
      });
      
      if (receipts && receipts.length > 0) {
        // Check if we need a new page for the receipts section
        checkPageBreak(20);
        
        doc.setFontSize(14);
        doc.setFont(undefined, 'bold');
        doc.setTextColor(0, 0, 0);
        doc.text('Receipt Attachments', 20, yPos);
        yPos += 10;
        ...
        for (let i = 0; i < receipts.length; i++) {
          ...
          try {
            ...
            doc.setFontSize(10);
            doc.setFont(undefined, 'bold');
            doc.text(`Receipt ${i + 1}:`, 25, yPos);
            doc.setFont(undefined, 'normal');
            doc.text(`${receipt.name} (${Math.round(receipt.size / 1024)}KB)`, 70, yPos);
            yPos += 7;
            
            // Check if the receipt has valid Base64 data
            if (receipt.data && typeof receipt.data === 'string' && receipt.data.length > 50 && receipt.data.includes('base64')) {
              // Check if we need a new page for the image
              checkPageBreak(120); // Reserve space for image
              
              // Try to get image dimensions and add image
              try {
                const dimensions = await getImageDimensions(receipt.data, 160, 100);
                
                // Add the image to PDF
                doc.addImage(
                  receipt.data,
                  receipt.type.includes('png') ? 'PNG' : 'JPEG',
                  25,
                  yPos,
                  dimensions.width,
                  dimensions.height
                ...
                doc.text('(Image could not be displayed - format not supported)', 25, yPos);
                doc.setTextColor(0, 0, 0);
                yPos += 7;
              }
            } else {
              ...
              const debugInfo = [];
              if (!receipt.data) debugInfo.push('data field missing');
              else if (typeof receipt.data !== 'string') debugInfo.push(`data type: ${typeof receipt.data}`);
              else if (receipt.data.length <= 50) debugInfo.push(`data too short: ${receipt.data.length} chars`);
              else if (!receipt.data.includes('base64')) debugInfo.push('not base64 format');
              
              console.warn('🔍 Receipt data issue:', {
                receiptName: receipt.name,
                issues: debugInfo,
                dataPreview: receipt.data ? `${typeof receipt.data}: ${String(receipt.data).substring(0, 100)}...` : 'null/undefined'
              });
              
              doc.setFontSize(9);
              doc.setTextColor(150, 150, 150);
              if (isOldClaim) {
                doc.text('(Receipt uploaded before image embedding was available)', 25, yPos);
              } else {
                doc.text(`(No image data available - ${debugInfo.join(', ')})`, 25, yPos);
              }
              doc.setTextColor(0, 0, 0);
              yPos += 7;
            }
            
            yPos += 5; // Space between receipts
          } catch (error) {
            console.warn('Error processing receipt:', error);
            doc.setFontSize(9);
            doc.setTextColor(200, 100, 100);
            doc.text(`(Error loading receipt ${i + 1})`, 25, yPos);
            doc.setTextColor(0, 0, 0);
            yPos += 7;
          }
        }
        ...
        doc.setFont(undefined, 'normal');
        doc.setTextColor(150, 150, 150);
        doc.text('No receipts attached to this claim', 25, yPos);
        doc.setTextColor(0, 0, 0);
        yPos += 15;
      }
      ...
      doc.setFontSize(8);
      doc.setTextColor(100, 100, 100);
      doc.text('This is a computer-generated document from RUBIX Attendance Management System', 20, pageHeight - 20);
      ...
      const receiptCount = (claim.receipts || claim.receiptData || []).length;
      const receiptText = receiptCount > 0 ? ` (including ${receiptCount} receipt${receiptCount > 1 ? 's' : ''})` : '';
      setSuccess(`Claim details exported successfully${receiptText}!`);
      setTimeout(() => setSuccess(''), 3000);
