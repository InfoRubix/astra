...
function CompanyAdminForgottenCheckouts() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      console.log('👤 Current company admin user:', user);
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      const requestsList = await forgottenCheckoutService.getForgottenCheckoutRequests(userCompany, statusFilter);
      console.log('Loaded company requests:', requestsList);
  ...
  const loadStats = async () => {
    try {
      ...
      const statsData = await forgottenCheckoutService.getForgottenCheckoutStats(userCompany, 30);
      console.log('Loaded company stats:', statsData);
      setStats(statsData);
    } catch (error) {
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Box>
              ...
              >
                Review and process forgotten check-out requests for {user.originalCompanyName || user.company || 'your company'}
              </Typography>
            </Box>
          </Box>
        </Box>
        ...
        <Box sx={{ display: { xs: 'block', md: 'none' } }}>
          {loading ? (
            ...
            <Box sx={{ 
              ...
              <Typography variant="body2" color="text.secondary">
                {statusFilter === 'pending' ? 'New forgotten checkout requests from your company will appear here for review' : 
                 statusFilter === 'approved' ? 'Approved requests from your company will be shown here' :
                 statusFilter === 'rejected' ? 'Rejected requests from your company will be displayed here' :
                 'Employee forgotten checkout requests from your company will appear here once submitted'}
              </Typography>
            </Box>
          ) : (
            <Box sx={{ p: 2 }}>
...
export default CompanyAdminForgottenCheckouts;